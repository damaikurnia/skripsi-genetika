...
//        ambilDatabase();
    ...
    public static Kromosom[] crossover(Kromosom[] parent, List<KelasKuliah> datakelaskuliah, List<MataKuliah> datamakul, List<tabelPermintaan> Permintaan) {
        ...
        for (int i = 0; i < krom[0].getData().length; i++) {
            if (i < point) { // samakan gen A dan C ke child 1 dan 2
                krom[14].getData()[i].setAllele(krom[0].getData()[i].getAllele());//2,8,14
                krom[15].getData()[i].setAllele(krom[1].getData()[i].getAllele());//3,9,15
            } else {
                krom[14].getData()[i].setAllele(krom[1].getData()[i].getAllele());//2,8,14
                krom[15].getData()[i].setAllele(krom[0].getData()[i].getAllele());//3.9,15
            }
            krom[14].getData()[i].setNilaiFitness(0);//2,8,14
            krom[15].getData()[i].setNilaiFitness(0);//3,9,15
        }
    ...
    public static Kromosom[] Mutasi(Kromosom[] parent, List<KelasKuliah> datakelaskuliah, List<MataKuliah> datamakul, List<tabelPermintaan> Permintaan) {
        ...
        for (int i = 14; i < krom.length; i++) { // 2-3 --> hanya kromosom anak yg di mutasi //2,8,14
